...
a2 = sum(accuracy_list2)/k
return(a2)
}
```

```{r}
standard_derivation <- function(data, accuracy){
  power_res <- c()
  for(i in 1:length(data)){
    power_res[i] = (data[i]-accuracy)*(data[i]-accuracy)
  }
  return(power_res)
}
```
...
sd <- standard_derivation(k_fold_result, hold_out_result)
sd
```